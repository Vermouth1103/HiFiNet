...
class GraphTransformerLayer(nn.Module):
    ...
    def forward(self, x, adj=None, attn_mask=None):
...
class GATLayer(nn.Module):
    ...
    def __init__(self, in_features, out_features, head_num, dropout=0.0, alpha=0.2):
        super(GATLayer, self).__init__()
        self.head_num = head_num
        self.out_features = out_features

        self.W = nn.Parameter(torch.empty(size=(in_features, head_num * out_features)))
        self.a = nn.Parameter(torch.empty(size=(2 * out_features, head_num)))
    ...
    def forward(self, h, adj):
        ...
        Wh = Wh.view(h.size(0), self.head_num, self.out_features)

        a_input = self._prepare_attentional_mechanism_input(Wh)
        e = self.leakyrelu(torch.einsum("ijk,kl->ijl", a_input, self.a))
        ...
        h_prime = torch.einsum("ijk,ikl->ijl", attention, Wh)
    ...
    def _prepare_attentional_mechanism_input(self, Wh):
        ...
        return all_combinations_matrix.view(N, N, -1, self.out_features * 2)
...
class RoutePlanModel(Module):
    ...
    def forward(self, train_segment_trajectories, destination):  # batch_size * length * dims
