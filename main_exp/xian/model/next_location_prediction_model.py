...
class NextLocationPredictionModel(Module):
    ...
    def __init__(self, config, segment_features, segment_adj_matrix, segment_dist_matrix, segment_sim_matrix, segment2locality_assignments, locality2region_assignments):
        super(NextLocationPredictionModel, self).__init__()
    ...
    def forward(self, train_segment_trajectories):  # batch_size * length * dims
        ...
        reconstructed_segment_features = self.multi_frequency_module(init_segment_features, updated_segment_features, self.segment_adj_matrix)
...
class MultiFrequencyModule(nn.Module):
    ...
    def forward(self, init_features, low_freq_features, segment_adj_matrix):
        ...
        edge_index = segment_adj_matrix.nonzero(as_tuple=False).t()
