...
class RPEEmbeddingEnhanced(nn.Module):
    
    def __init__(self, config):
        super(RPEEmbeddingEnhanced, self).__init__()
        self.config = config
        self.dist_embedding = nn.Embedding(config.rpe_dist_num, config.rpe_dims)
        self.sim_embedding = nn.Embedding(config.rpe_sim_num + 1, config.rpe_dims)
        self.proj_rpe = nn.Linear(config.rpe_dims * 2, 1)

    def forward(self, dist_matrix, sim_matrix):
        dist_indices = (dist_matrix + 1).long()

        sim_indices = (sim_matrix * (self.config.rpe_sim_num)).long()

        dist_embedded = self.dist_embedding(dist_indices)
        sim_embedded = self.sim_embedding(sim_indices)

        fused_rpe = torch.cat([dist_embedded, sim_embedded], dim=-1)

        rpe_scores = self.proj_rpe(fused_rpe).squeeze(-1)
        return rpe_scores
...
class LocalTransformerLayer(nn.Module):
    ...
    def __init__(self, hidden_dims, head_num):
        ...
        self.multihead_attn = nn.MultiheadAttention(hidden_dims, head_num, batch_first=True)
    ...
    def forward(self, node_features, assignments):
        
        updated_features = torch.zeros_like(node_features)
        unique_groups = torch.unique(assignments)
        
        for group in unique_groups:
            group_nodes = (assignments == group).nonzero(as_tuple=True)[0]
            if group_nodes.numel() == 0:
                continue
            
            group_features = node_features[group_nodes]
            
            group_features = group_features.unsqueeze(1)
            
            attn_output, _ = self.multihead_attn(group_features, group_features, group_features)
            
            attn_output = attn_output.squeeze(1)
            
            updated_features[group_nodes] = attn_output
        
        x = self.norm1(node_features + updated_features)
        ff_output = self.linear2(F.relu(self.linear1(x)))
        x = self.norm2(x + ff_output)
        return x
...
class SoftAdaptiveLocalityPooling(nn.Module):
    ...
    def forward(self, segment_features):
        ...
        locality_features = torch.matmul(segment2locality_assignments, values) + self.locality_centers
...
class HiFiRoad(nn.Module):
    ...
    def forward(self, segment_features, segment_adj_matrix, dist_matrix, sim_matrix):
        ...
        return init_segment_features, high_freq_features, reconstructed_segment_features, updated_locality_features, updated_region_features, \
...
class HierarchicalGNNModule(nn.Module):
    ...
    def forward(self, init_segment_features, segment_adj_matrix, rpe):
        ...
        locality_updates = torch.matmul(locality2region_assignments.T, updated_region_features)  # [locality_num, hidden_dims]
        ...
        segment_updates = torch.matmul(segment2locality_assignments.T, updated_locality_features)
