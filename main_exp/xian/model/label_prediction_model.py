...
class LabelPredictionModel(nn.Module):
    ...
    def __init__(self, config, segment_features, segment_dist_matrix, segment_sim_matrix, segment2locality_assignments, locality2region_assignments):
        ...
        self.hierarchical_gnn_module = HierarchicalGNNModule(config, segment2locality_assignments, locality2region_assignments)
        
        self.multi_frequency_module = MultiFrequencyModule(config)

        self.classifier = nn.Linear(config.hidden_dims*2, 1)

        self.sigmoid = nn.Sigmoid()

    def forward(self, train_segments):
        ...
        segment_features = torch.cat((
            reconstructed_segment_features[train_segments], init_segment_features[train_segments]), -1)

        logits = self.classifier(segment_features)
...
class HierarchicalGNNModule(nn.Module):
    ...
    def __init__(self, config, segment2locality_assignments, locality2region_assignments):
    ...
    def forward(self, init_segment_features, rpe):
        ...
        locality_features = torch.matmul(self.segment2locality_assignments, segment_features)

        region_features = torch.matmul(self.locality2region_assignments, locality_features)

        updated_region_features = self.region_transformer(region_features.unsqueeze(0)).squeeze(0)

        locality_updates = torch.matmul(self.locality2region_assignments.T, updated_region_features)  # [locality_num, hidden_dims]
        locality_update_ratio = self.locality_update_gate(torch.cat((locality_features, locality_updates), dim=-1))
        locality_features = (1 - locality_update_ratio) * locality_features + locality_update_ratio * locality_updates
        updated_locality_features = self.locality_transformer(locality_features.unsqueeze(0)).squeeze(0)

        segment_updates = torch.matmul(self.segment2locality_assignments.T, updated_locality_features)
        segment_update_ratio = self.segment_update_gate(torch.cat((segment_features, segment_updates), dim=-1))
        segment_features = (1 - segment_update_ratio) * segment_features + segment_update_ratio * segment_updates
        updated_segment_features = self.segment_transformer(segment_features.unsqueeze(0)).squeeze(0)
...
class MultiFrequencyModule(nn.Module):
    ...
    def forward(self, init_features, low_freq_features):
        ...
        updated_low_freq_features = self.low_freq_module(low_freq_features)
        updated_high_freq_features = self.high_freq_module(high_freq_features)
