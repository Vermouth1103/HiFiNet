...
def train_next_location_prediction(args):
    ...
    for epoch in tqdm(range(config.next_location_prediction_epochs)):
        ...
        if acc1 > max_acc1 or acc5 > max_acc5:
            ...
            output_next_location_prediction_dir = os.path.join(config.output_dir, "next_location_prediction_model")
            if not os.path.exists(output_next_location_prediction_dir):
                os.makedirs(output_next_location_prediction_dir)
            torch.save(model.state_dict(), os.path.join(output_next_location_prediction_dir, f"next_location_prediction_model_{args.timestamp}"))
