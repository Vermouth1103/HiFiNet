...
def load_hierarchy_construct_data(config):
    ...
    while len(segment_features) < config.segment_num:
        segment_features.append([0, 0, 0, 0])
    segment_features = np.array(segment_features, dtype=np.int32)

    with open(config.segment_adj, "rb") as f:
        segment_adj_matrix = np.array(pickle.load(f))

    with open(config.segment_trajectory_adj, "rb") as f:
        segment_trajectory_adj_matrix = np.array(pickle.load(f))
    ...
    return segment_features, segment_adj_matrix, segment_trajectory_adj_matrix, segment_dist_matrix, segment_sim_matrix, \
...
def load_downstream_task_data(config):
    ...
    with open(config.segment2locality_assign, "rb") as f:
        segment2locality_assign = np.array(pickle.load(f))
